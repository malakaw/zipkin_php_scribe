...
class TPhpStream extends TTransport {
  ...
  public function read($len) {
    ...
    if ($data === FALSE ) {
      throw new TException('TPhpStream: Could not read '.$len.' bytes [data === FALSE]');
    }
    if ($data === '') {
      throw new TException('TPhpStream: Could not read '.$len.' bytes [data is null]');
    }
    return $data;
  }
